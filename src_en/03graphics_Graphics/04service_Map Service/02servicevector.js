...
const queryMapData = async () => {
    ...
    res = await svc.conditionQueryFeature({
        condition: `1 == 1`, // SQL WHERE clause; see "server condition query" docs for fields
        fields: "",
        includegeom: true, // Return geometry; when false, multiple results get bbox only, single gets full GeoJSON
        realgeom: true,
        limit: 1000000 // Omit for default 100
    }, pt => {
        // Transform each queried point
        return cadPrj.fromMercator(pt);// To CAD coords
    })
    if (res.error) {
        showError(res.error)
        return
