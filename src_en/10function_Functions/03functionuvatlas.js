...
// -- Auto UV atlas for geometry --
// Split model surface into charts and pack into one or more textures
...
objects.forEach(function(child) {
    if (child.isMesh && child.geometry && child.visible) {
        if (!child.geometry.index) {
            // If no vertex index, use mergeVertices to generate
...
// If geometry has no UVs, unwrap to generate UVs
for (const geom of geoms) {
    if (!geom.attributes.uv) await unwrapper.unwrapGeometry(geom);
}

// Unwrap all geometry onto one UV atlas and assign UVs
