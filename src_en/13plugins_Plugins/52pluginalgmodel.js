...
ent.addTo(app);
//
app.logInfo("Generating 2D plan from 3D model... (Export to DWG from top-right when done)", 10000)
let model = ent.node;
...
for(let n = 0; n < positions.length; n+=6) {
    ...
    if (vjmap3d.approxArrayEquals(start, end)) continue; // Coincident points
    entitys.push({
        typename: "DbLine",
        start: start,
        end: end,
    ...
    buttons: [
        {
            ...
            onclick: async () => {
                let res = await app.svc.updateMap({
                    ...
                    style: {
                        backcolor: 0 // Use 0xFFFFFF for light background
                    }
                })
