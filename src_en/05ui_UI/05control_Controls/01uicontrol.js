...
const ui = await app.getConfigPane({ title: "Tools", style: { width: "300px"}});
let gui = new vjmap3d.UiPanelJsonConfig();
gui.add(controls, "showMousePositionControl").label("Mouse position").onChange(v => {
    if (v) {
...
gui.add(controls, "showFullscreenControl").label("Fullscreen").onChange(v => {
    if (v) {
...
gui.add(controls, "showButtonGroupControl").label("Button group").onChange(v => {
    if (v) {
...
gui.add(controls, "showMiniMapControl").label("Mini map").onChange(v => {
    if (v) {
